...
| Motor direction     | icon (↻) on each disc opening a menu          | `MSP2_SEND_DSHOT_COMMAND` (0x3003): blocking, `DSHOT_CMD_SPIN_DIRECTION_1` (7) / `_2` (8) + `SAVE_SETTINGS` (12) | Normal / Reversed       | Only while motor control is enabled and the FC runs a DShot protocol; every motor is stopped first. The ESC stores it, no reboot. Nothing reports the current direction back, so both are offered and a note says to spin the motor and pick the other one if it still turns the wrong way                                                           |
| Swap motor          | icon (⇄) on each disc opening a menu          | `motor_output_reordering` · `MSP2_MOTOR_OUTPUT_REORDERING` (0x3001) / SET (0x3002)                               | the other motors        | Exchanges the ESC outputs of the two motors in the draft: **Save & Reboot**. Remapped motors are listed under the drawing ("Motor 1 drives ESC output 3 · …"); nothing is shown for the default order                                                                                                                                                |
...
- Mixer type, Betaflight's step-by-step reordering / direction wizards (replaced by the per-motor menus), 3D mode,
...
- [x] Direction menu locked until motor control is on; "Reversed" stops the motors, sends 8 + 12 to that ESC
      (blocking) and the mock ESC's stored direction flips (visible on the ESC tab); nothing is sent on a
      non-DShot protocol
- [x] Swap motor 1 with 3 → hint "Motor 1 drives ESC output 3 · Motor 3 drives ESC output 1 — Save & Reboot to
      apply", test locked; Save & Reboot persists `[2, 1, 0, 3, …]` and the hint stays; swapping back clears it
...
- [ ] "Reversed" on a motor makes it spin the other way on the next slider move, and still after a power cycle
...
- Direction via DShot commands (like the Betaflight Configurator's wizard) rather than through the ESC settings
  ...
  price is that the current direction can't be read back, hence Normal / Reversed instead of a toggle.
