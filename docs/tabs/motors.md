...
| ------------------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Recommendations for | button group under the page header                     | — (not written to the FC)                                                                                        | Whoop / 3" / **5"** / 7"       | Picks the dynamic idle slider range and zones and the "Good for a …" texts, see below. By prop size, not battery voltage. Shows the chosen type's description next to it. Kept for the session                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Dynamic idle        | slider with zone band                                  | `dyn_idle_min_rpm` (×100 rpm) · read `MSP_PID_ADVANCED` (94) byte 49, written by name via `MSP2_CLI_SETTING`     | per drone type · FC value      | Range and zones per drone type, see below. Disabled without bidirectional DShot (it needs RPM data). FC value 0 is shown as "Off"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Motor idle          | slider with zone band                                  | `motor_idle` (0.01 %) · `MSP_ADVANCED_CONFIG` (90/91, bytes 6–7, u16)                                            | 2–12 %, 0.1 % steps · FC value | Same zones for every drone type, see below. Shown in %. An FC value outside the slider is shown as it is and only replaced once the slider is moved                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
Dynamic idle slider and zones (`DYN_IDLE_SCALES` in `src/lib/motors/model.ts`, ×100 rpm). Status is always icon + text,
never colour alone. 5" is the user's; the others are estimates (see Decisions) until checked on real quads:

| Drone type                            | Slider | Good  | Warning      | Danger       |
| ------------------------------------- | ------ | ----- | ------------ | ------------ |
| Whoop (65–75 mm, 1–2S)                | 30–80  | 45–60 | 39–44, 61–66 | 30–38, 67–80 |
| 3" (2.5–3.5": toothpicks, cinewhoops) | 16–50  | 28–36 | 24–27, 37–40 | 16–23, 41–50 |
| **5"** (freestyle and racing)         | 12–40  | 18–25 | 15–17, 26–28 | 12–14, 29–40 |
| 7" (6–7" long range)                  | 8–32   | 14–20 | 11–13, 21–23 | 8–10, 24–32  |

An edited value must lie within 8–80 (all sliders together); an untouched FC value outside the chosen type's slider
is shown as "Set to …, outside this slider".

Motor idle zones (`MOTOR_IDLE_ZONES`), in %, the same for every drone type:

| Good | Warning       | Danger                   |
| ---- | ------------- | ------------------------ |
| 4–8  | 3–3.9, 8.1–10 | the rest: 2–2.9, 10.1–12 |
...
- [x] Drone type picker: 5" pressed by default; Whoop moves the dynamic idle slider to 30–80 and the texts to
      "for a whoop"; switching type never changes a value (50 set as a whoop shows "outside this slider" on 5")
...
- Drone type picker (user request, 2026-09-26, asked for 1–2S / 3–4S / 6S): built by prop size instead, which the
  user asked to be corrected on. Builders choose KV to suit the voltage, so a 5" on 4S (~2400 KV) and on 6S
  (~1800 KV) turn about the same RPM and want the same idle; 1–2S whoops land together too, while "3–4S" would
  span 2.5" to 7". The non-5" dynamic idle zones are estimates of each class's RPM at the default 5.5 % static
  idle (KV × volts × idle: 1S 0802 ~25 000 KV ≈ 5000 rpm, 3" 4S ~3600 KV ≈ 3200, 7" 6S ~1300 KV ≈ 1700), which
  gives 21 for a 5" — inside the user's 18–25. Motor idle keeps one set of zones: a share of full throttle already
  scales with KV × volts.
- The picker only changes what the sliders recommend and is kept in memory for the session
  (`src/stores/droneType.ts`). Remembering it per FC, and applying defaults, is the drone-type setup step in SPEC §2.
