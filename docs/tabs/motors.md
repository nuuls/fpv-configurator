...
| Motor direction     | icon (↻) on each disc: one click flips it              | `MSP2_SEND_DSHOT_COMMAND` (0x3003): blocking, `DSHOT_CMD_SPIN_DIRECTION_1` (7) / `_2` (8) + `SAVE_SETTINGS` (12) | —                       | Only while motor control is on and the FC runs a DShot protocol. Like the Betaflight Configurator's wizard: all motors stop, 500 ms pause (ESCs drop commands while the motor turns), command, 500 ms, then that motor spins at the FC's idle throttle (`motor_idle`, `MSP_ADVANCED_CONFIG` bytes 6–7; 5.5 % → 1055, kept within 3–10 %) for 2 s so the result shows — the Configurator's wizard spins at idle too. The ESC stores it, no reboot. Nothing reports the current direction, so the first click sends "reversed" and each click sends the opposite of the last one; the note says "Still the wrong way? Click again" |
