...
export function DiffPage() {
  ...
  const state: ReadState =
    result && result.client === client && result.generation === generation
      ? result.state
      : { phase: 'reading' }
  ...
  useEffect(() => {
    ...
    readDiff(client).then(
      ...
      (cause: unknown) =>
        finish({ phase: 'failed', message: `Could not read the diff: ${describeError(cause)}` }),
    )
    return () => {
      cancelled = true
    }
    ...
    <>
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-4 text-sm">
          <Button
            disabled={!client || state.phase === 'reading'}
      ...
      {state.phase === 'done' && (
        <DiffReportView
          report={showHidden ? state.report : tuningOnly(state.report)}
          all={showHidden}
        />
      )}
    </>
...
function summary(report: DiffReport, showHidden: boolean): string {
  ...
  return [
    differences,
    hidden > 0 && `${hidden} other ${showHidden ? 'shown' : 'hidden'}`,
    report.board,
    report.firmware,
  ]
    .filter(Boolean)
  ...
  return (
    <>
      ...
      {report.sections.length === 0 && report.errors.length === 0 && (
        <Notice>
          {all
            ? 'No differences — every setting is at its default.'
            : 'No tuning differences — PIDs, rates and filters are at their defaults.'}
        </Notice>
      )}
...
function SectionDiff({ section }: { section: DiffSection }) {
  const count = section.entries.filter(
    (entry) => entry.kind === 'setting' || !entry.isDefault,
  ).length
  return (
    <section
      role="group"
      aria-label={section.title}
      className="bg-card overflow-hidden rounded-lg border font-mono text-sm"
    >
      <header className="bg-muted/50 flex items-baseline justify-between gap-4 border-b px-3 py-2">
      ...
      <ul className="py-1">
        {section.entries.map((entry, index) => (
          <li key={index} className="hover:bg-muted/50 px-3 py-0.5">
