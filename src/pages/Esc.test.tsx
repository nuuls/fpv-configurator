...
describe('ESC tab', () => {
  ...
  it('reads all four ESCs and shows them as one, because they are set up alike', async () => {
    ...
    expect(within(all).getByText(/48 kHz build. Flight performance is greatly reduced/)).toBeInTheDocument()
  ...
  it('writes changed settings to all ESCs after asking, and they are still there when the ESCs are read again', { timeout: 20_000 }, async () => {
    const user = await openTab('ESC')
    await user.click(screen.getByRole('button', { name: 'Read ESCs' }))
    await user.selectOptions(within(await combinedCard()).getByLabelText('Motor timing'), '15° (medium)')
    ...
    expect(await screen.findByText(/ESC 1, 2, 3, 4 will be changed and restarted/)).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Cancel' }))
    expect(screen.getByRole('button', { name: 'Save' })).toBeEnabled()

    await user.click(screen.getByRole('button', { name: 'Save' }))
    await user.click(await screen.findByRole('button', { name: 'Write settings' }))
    await waitFor(() => expect(screen.getByRole('button', { name: 'Save' })).toBeDisabled(), { timeout: 5000 })
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: 'Read again' })).toBeEnabled(), { timeout: 5000 })
    const all = await combinedCard()
...
describe('ESCs that are not alike', () => {
  it('shows firmware, version, hardware and settings of every ESC on its own card', async () => {
    ...
    expect(within(screen.getByRole('group', { name: 'Bluejay settings' })).getByText('ESC 1, 2 — set up alike')).toBeInTheDocument()
    expect(within(screen.getByRole('group', { name: 'AM32 settings' })).getByLabelText('Motor KV')).toHaveValue(2220)
  ...
  it('offers every setting of the AM32 configurator, the motor direction per ESC', async () => {
    ...
    render(<Editable reports={await readReports([mockAm32Esc(), patched(mockAm32Esc(), 0x7c00, { 17: 1 })])} />)
    const all = screen.getByRole('group', { name: 'All ESCs' })
    for (const section of ['Essentials', 'Motor', 'Extended settings', 'Limits', 'Current control', 'Sinusoidal startup', 'Brake', 'Servo input']) {
  ...
  it('shows ESC 1\'s values when editable settings differ, and can use them for all', async () => {
    const user = userEvent.setup()
    render(<Editable reports={await readReports([mockBluejayEsc(), patched(mockBluejayEsc(), 0x1a00, { 0x15: 2 })])} />)
  ...
  it('tells to update ESCs whose firmware version it does not work with, and offers nothing to change on them', async () => {
    ...
    render(<EscReportList reports={await readReports([old(), old(), patched(mockAm32Esc(), 0x7c00, { 4: 18 })])} />)
    expect(screen.getAllByRole('alert').map((alert) => alert.textContent)).toEqual([
      expect.stringMatching(/Bluejay 0.19 is too old: this app only works with Bluejay 0.21. Update the ESCs/),
