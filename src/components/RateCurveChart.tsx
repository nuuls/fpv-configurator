  ...
  return (
    <figure className="relative w-full">
        ...
        {series.map((s) => (
          <li key={s.axis} className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground tabular-nums">
              max {Math.round(rateAt(s.type, s.rates, 1, s.limit))}°/s
            </span>
          </li>
        ))}
      </ul>
      ...
      <svg
        ...
        className="w-full"
        role="img"
        aria-label="Rate curve: rotation speed over stick deflection"
      >
        {/* recessive grid + axes */}
          ...
          <g pointerEvents="none">
            ...
            {series.map((s) => (
              <circle
                key={s.axis}
                cx={x(hoverStick)}
                cy={y(rateAt(s.type, s.rates, hoverStick, s.limit))}
                r={4.5}
                fill={color(s)}
                stroke="var(--card)"
                strokeWidth={2}
      ...
      {hoverStick !== null && (
        <div
          role="tooltip"
          className="bg-popover pointer-events-none absolute top-10 z-10 rounded-md border px-3 py-2 text-xs shadow-md"
          style={
            hoverStick < 0.55
              ? { left: `${(x(hoverStick) / WIDTH) * 100 + 3}%` }
              : { right: `${100 - (x(hoverStick) / WIDTH) * 100 + 3}%` }
          }
        >
          <div className="text-muted-foreground mb-1">{Math.round(hoverStick * 100)}% stick</div>
          {series.map((s) => (
            <div key={s.axis} className="flex items-center gap-2 tabular-nums">
              ...
              <span className="ml-auto pl-3 font-medium">
                {Math.round(rateAt(s.type, s.rates, hoverStick, s.limit))}°/s
              </span>
            </div>
          ))}
        </div>
