...
export async function readSetupSnapshot(client: MspClient): Promise<SetupSnapshot> {
  ...
  return {
    ...
    accCalibrated:
      (board.configurationProblems & CONFIGURATION_PROBLEM.ACC_NEEDS_CALIBRATION) === 0,
    ...external,
  }
}
 ...
 */
async function readExternalChanges(
  client: MspClient,
): Promise<Pick<SetupSnapshot, 'external' | 'externalError'>> {
  try {
...
export async function saveSetup(
  ...
  const beeperChanged =
    draft.beeperOffFlags !== saved.beeperOffFlags ||
    draft.dshotBeaconOffFlags !== saved.dshotBeaconOffFlags
  ...
  if (draft.airmode !== saved.airmode)
    await writeFeatures(client, withAirmode(snapshot.features, draft.airmode))
