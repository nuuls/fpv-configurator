...
describe('decodeDeviceInfo / readPlan', () => {
  ...
  it('knows where the SiLabs MCUs keep their settings', () => {
    ...
    expect(plan(0xe8b1)).toMatchObject({ family: 'silabs', mcu: 'EFM8BB10', settingsAddress: 0x1a00, settingsLength: 0xff, erasePage: 13, writable: true })
  ...
  it('maps the AM32 flash size codes to the settings address, shifted for 128 k parts', () => {
    ...
    expect(plan(0x1f06)).toMatchObject({ family: 'arm', settingsAddress: 0x7c00, fileNameAddress: 0x7be0, settingsLength: 0xb8, erasePage: null, writable: true })
    ...
    expect(plan(0x2b06)).toMatchObject({ settingsAddress: 0x7e00, fileNameAddress: 0x7df8, writable: false })
  })
...
describe('describeEsc', () => {
  it('decodes Bluejay 0.21 (layout 208)', () => {
    ...
    expect(report).toMatchObject({ status: 'ok', firmware: 'Bluejay', version: '0.21.0', hardware: 'Z-H-30 · EFM8BB21', layoutRevision: 208, note: null, editable: true })
  ...
  it('only takes Bluejay 0.21: older ones have to be updated, and the patch level comes from the name', () => {
    ...
    expect(old).toMatchObject({ status: 'unsupported', firmware: 'Bluejay', version: '0.20.1 RC2', hardware: 'Z-H-30 · EFM8BB21' })
    expect(old).toMatchObject({ description: expect.stringMatching(/too old.*Update the ESCs/) })
    expect(describeEsc(rawRead(mockBluejayEsc(), patch))).toMatchObject({ status: 'ok', version: '0.21.1 RC2' })

    const newer = describeEsc(rawRead(mockBluejayEsc(), { 0x01: 22, 0x02: 209 }))
    expect(newer).toMatchObject({ status: 'unsupported', version: '0.22.0', description: expect.stringContaining('newer') })
  })

  it('warns about a Bluejay that is not the 24 kHz build', () => {
    expect(describeEsc(rawRead(mockBluejayEsc()))).toMatchObject({ warnings: [expect.stringMatching(/48 kHz build.*greatly reduced/)] })
    expect(describeEsc(rawRead(mockBluejayEsc({ pwmKhz: 96 })))).toMatchObject({ warnings: [expect.stringContaining('96 kHz')] })
  ...
  it('decodes BLHeli_S 16.7 (layout 33)', () => {
    ...
    expect(report).toMatchObject({ status: 'ok', firmware: 'BLHeli_S', version: '16.7', hardware: 'A-H-30 · EFM8BB10', editable: false, warnings: [] })
    expect(settingsOf(report)).toEqual({
  ...
  it('decodes AM32 2.21 (eeprom version 4) with every setting of the AM32 configurator', () => {
    ...
    expect(report).toMatchObject({ status: 'ok', firmware: 'AM32', version: '2.21', hardware: 'MOCK_ESC_F051', layoutRevision: 4, editable: true })
    expect(settingsOf(report)).toEqual({
      'Motor direction': 'Normal',
  ...
  it('only takes AM32 2.21, and pads the minor version', () => {
    ...
    expect(old).toMatchObject({ status: 'unsupported', firmware: 'AM32', version: '2.05', hardware: 'MOCK_ESC_F051' })
    expect(old).toMatchObject({ description: expect.stringContaining('only works with AM32 2.21') })
    expect(describeEsc(rawRead(mockAm32Esc(), { 4: 22 }))).toMatchObject({ status: 'unsupported', version: '2.22' })
  })

  it('falls back to the MCU without an AM32 file name, and only reads ESCs with 128 k flash', () => {
    const raw = rawRead(mockAm32Esc())
    expect(describeEsc({ ...raw, fileName: null })).toMatchObject({ hardware: 'ARM, 32 k flash', editable: true })
    expect(describeEsc({ ...raw, plan: { ...raw.plan, writable: false } })).toMatchObject({ status: 'ok', editable: false })
  })
  ...
  it('shows firmware and version but no settings for a layout it does not know', () => {
    ...
    expect(report).toMatchObject({ status: 'ok', firmware: 'Bluejay', settings: [], note: expect.stringContaining('215'), editable: false, warnings: [] })
  })
...
describe('combineReports', () => {
  ...
  it('keeps the ESCs apart when firmware, version or hardware differ', () => {
    ...
    expect(combineReports([bluejay(), bluejay(undefined, { 0x01: 20 })])).toEqual({ view: 'separate', reason: null })
...
describe('editing', () => {
  ...
  const mixed = () => read([mockBluejayEsc(), mockBluejayEsc({ reversed: true }), mockBlheliSEsc(), mockAm32Esc()])
  ...
  it('groups the ESCs by firmware and lists what can be changed: little on Bluejay, everything on AM32, nothing on BLHeli_S', () => {
    ...
    expect(groups[0]?.settings.map((def) => def.key)).toEqual(['startupPowerMin', 'startupPowerMax', 'timing'])
    expect(groups[1]?.settings).toHaveLength(39)
    expect(groups[1]?.settings.filter((def) => def.perMotor).map((def) => def.key)).toEqual(['direction'])
  })
  ...
  it('converts numbers the way ESC Configurator and the AM32 configurator show them', () => {
    ...
    expect([numberToRaw(min, 1025), numberToRaw(min, 1125), numberToRaw(min, 5000), numberToRaw(min, 0)]).toEqual([51, 255, 255, 0])
    const max = numberControl(reports, 'Bluejay', 'startupPowerMax')
    expect([rawToNumber(max, 5), numberToRaw(max, 1300), numberToRaw(max, 9999), numberToRaw(max, 1)]).toEqual([1020, 75, 75, 1])

    const kv = numberControl(reports, 'AM32', 'motorKv')
    expect([rawToNumber(kv, 55), numberToRaw(kv, 1950), numberToRaw(kv, 1960)]).toEqual([2220, 48, 49])
    const cell = numberControl(reports, 'AM32', 'lowVoltageThreshold')
    expect([rawToNumber(cell, 50), numberToRaw(cell, 3.3), numberToRaw(cell, 9)]).toEqual([3, 80, 100])
    const ramp = numberControl(reports, 'AM32', 'maxRamp')
    expect([rawToNumber(ramp, 160), numberToRaw(ramp, 0.1), numberToRaw(ramp, 0)]).toEqual([16, 1, 1])
  })
  ...
  it('greys out AM32 settings that another one makes meaningless', () => {
    ...
    expect([enabled('timing', { autoAdvance: 1 }), enabled('timing', { autoAdvance: 0 })]).toEqual([false, true])
    expect([enabled('pwmFrequency', { variablePwm: 2 }), enabled('pwmFrequency', { variablePwm: 1 })]).toEqual([false, true])
    expect([enabled('lowVoltageThreshold', { lowVoltageCutoff: 1 }), enabled('absoluteVoltageCutoff', { lowVoltageCutoff: 1 })]).toEqual([true, false])
    expect([enabled('activeBrakePower', { brakeOnStop: 2 }), enabled('brakeStrength', { brakeOnStop: 1, rcCarReversing: 1 })]).toEqual([true, false])
  })

  it('finds shared settings that are not the same on all ESCs of a group and can level them', () => {
    const reports = [describeEsc(rawRead(mockBluejayEsc())), describeEsc(rawRead(mockBluejayEsc({ reversed: true }), { 0x15: 2, 0x07: 9 }))]
  ...
  it('describes an ESC again from the block that was written to it', () => {
    ...
    expect(after).toMatchObject({ firmware: 'Bluejay', version: '0.21.0', editable: true, warnings: esc.warnings })
  })
})
