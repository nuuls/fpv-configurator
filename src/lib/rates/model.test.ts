...
describe('rates payload', () => {
  ...
  it('validates ranges', () => {
    ...
    expect(
      validateRates({ type: 3, axes: [{ ...DEFAULT, rate: 2500 }, DEFAULT, DEFAULT] })[0],
    ).toMatch(/Roll max rate/)
    expect(
      validateRates({ type: 3, axes: [DEFAULT, DEFAULT, { ...DEFAULT, expo: 120 }] })[0],
    ).toMatch(/Yaw expo/)
  })
})
...
describe('axis sync', () => {
  ...
  it('switching sync on copies roll to the followers', () => {
    ...
    expect(applySync([DEFAULT, different, different], 'off')).toEqual([
      DEFAULT,
      different,
      different,
    ])
...
const actual = (rates: AxisRates, stick: number, limit?: number) =>
  rateAt(RATES_TYPE.ACTUAL, rates, stick, limit)
...
describe('Actual rates curve', () => {
  ...
  it('draws one curve per distinct setting, coloured by its first axis', () => {
    expect(
      curveSeries({ type: 3, axes: [DEFAULT, DEFAULT, DEFAULT] }, [1998, 1998, 1998]).map(
        (s) => s.label,
      ),
    ).toEqual(['Roll · Pitch · Yaw'])
    const split = curveSeries(
      { type: 3, axes: [DEFAULT, DEFAULT, { ...DEFAULT, rate: 500 }] },
      [1998, 1998, 1998],
    )
    expect(split.map((s) => [s.label, s.axis])).toEqual([
      ['Roll · Pitch', 0],
      ['Yaw', 2],
    ])
  })
...
describe('other rate types', () => {
  ...
  it("starts every type from Betaflight Configurator's defaults, on all three axes", () => {
    ...
    for (const type of Object.keys(RATES_TYPES))
      expect(validateRates(defaultRates(Number(type)) ?? { type: 9, axes: [] })).toEqual([])
  })

  it('names the three numbers per type', () => {
    const labels = (type: number) =>
      Object.values(RATES_TYPES[type]?.fields ?? {}).map(
        (f) => `${f.label}${f.unit && ` (${f.unit})`}`,
      )
    ...
    expect(labels(RATES_TYPE.ACTUAL)).toEqual([
      'Center sensitivity (°/s)',
      'Max rate (°/s)',
      'Expo',
    ])
    expect(labels(RATES_TYPE.QUICK)).toEqual(['RC rate', 'Max rate (°/s)', 'Expo'])
    const kiss = RATES_TYPES[RATES_TYPE.KISS]?.fields
    expect(kiss && [fieldName(kiss.rcRate, 0), fieldName(kiss.rate, 2)]).toEqual([
      'Roll RC rate',
      'Yaw rate',
    ])
  })
  ...
  it('stores °/s fields / 10 and everything else as the plain byte, depending on the type', () => {
    ...
    expect(bytes(RATES_TYPE.BETAFLIGHT, { rcRate: 120, rate: 75, expo: 20 })).toEqual([
      0, 120, 75, 20,
    ])
    expect(bytes(RATES_TYPE.RACEFLIGHT, { rcRate: 370, rate: 80, expo: 50 })).toEqual([
      1, 37, 80, 50,
    ])
  ...
  it('validates against the limits of the type (ratesSettingLimits)', () => {
    const invalid = (type: number, rates: Partial<AxisRates>) =>
      validateRates({ type, axes: [{ ...start(type), ...rates }] })[0]
    expect(invalid(RATES_TYPE.BETAFLIGHT, { rcRate: 256 })).toBe(
      'Roll RC rate must be between 0.01 and 2.55.',
    )
    expect(invalid(RATES_TYPE.BETAFLIGHT, { rate: 101 })).toBe(
      'Roll super rate must be between 0.00 and 1.00.',
    )
    expect(invalid(RATES_TYPE.RACEFLIGHT, { rcRate: 2010 })).toBe(
      'Roll rate must be between 10°/s and 2000°/s.',
    )
    expect(invalid(RATES_TYPE.RACEFLIGHT, { rate: 256 })).toBe(
      'Roll acro+ must be between 0 and 255.',
    )
    ...
    expect(invalid(RATES_TYPE.QUICK, { rcRate: 0 })).toBe(
      'Roll RC rate must be between 0.01 and 2.55.',
    )
  })
  ...
  it('Betaflight rates: 200°/s × RC rate, divided by (1 − stick × super rate); RC rate above 2.0 grows faster', () => {
    ...
    expect(rateAt(RATES_TYPE.BETAFLIGHT, { rcRate: 100, rate: 0, expo: 50 }, 0.5)).toBeCloseTo(
      200 * (0.5 ** 4 * 0.5 + 0.25),
      6,
    )
    expect(rateAt(RATES_TYPE.BETAFLIGHT, { rcRate: 210, rate: 0, expo: 0 }, 0.1)).toBeCloseTo(
      20 * (2.1 + 14.54 * 0.1),
      6,
    )
  ...
  it('Raceflight rates: rate × (1 + stick × Acro+ %), cubic expo', () => {
    ...
    expect(rateAt(RATES_TYPE.RACEFLIGHT, rates, 0.5)).toBeCloseTo(
      370 * (1 + 0.5 * (0.25 - 1)) * 0.5 * 1.4,
      6,
    )
  })
  ...
  it('KISS rates: 2000°/s × RC rate / 10, divided by (1 − stick × rate)', () => {
    ...
    expect(rateAt(RATES_TYPE.KISS, { rcRate: 100, rate: 70, expo: 30 }, 0.5)).toBeCloseTo(
      (2000 * 0.1 * (0.125 * 0.3 + 0.5 * 0.7)) / 0.65,
      6,
    )
  })
  ...
  it('Quick rates: RC rate × 200°/s around the center, reaching max rate at full stick', () => {
    ...
    expect(rateAt(RATES_TYPE.QUICK, { rcRate: 100, rate: 670, expo: 100 }, 0.5)).toBeCloseTo(
      100 / (1 - 0.125 * (470 / 670)),
      6,
    )
...
describe('rates I/O', () => {
  it('saves without needing a reboot and survives one', async () => {
    ...
    const draft = {
      type: 3,
      axes: [
        { rcRate: 90, rate: 720, expo: 15 },
        { rcRate: 90, rate: 720, expo: 15 },
        { rcRate: 80, rate: 600, expo: 0 },
      ],
    }
    ...
    const quick = {
      type: 4,
      axes: [
        { rcRate: 120, rate: 800, expo: 25 },
        { rcRate: 120, rate: 800, expo: 25 },
        { rcRate: 90, rate: 500, expo: 0 },
      ],
    }
