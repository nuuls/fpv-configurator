...
const close = (v: Vec3, expected: Vec3) =>
  v.forEach((value, i) => expect(value).toBeCloseTo(expected[i] ?? NaN, 6))
...
describe('renderQuad', () => {
  ...
  it('sorts far-to-near and shows the top of a level board', () => {
    ...
    expect(polygons.map((p) => p.depth)).toEqual(
      [...polygons.map((p) => p.depth)].sort((a, b) => b - a),
    )
  ...
  it('paints the mark and the arrow on top of the board, whichever way the quad and the board are turned', () => {
    for (let yaw = 0; yaw < 360; yaw += 15) {
      for (const attitude of [
        { ...level, yaw },
        { roll: 25, pitch: -20, yaw },
        { roll: -40, pitch: 35, yaw },
      ]) {
        for (const alignment of [
          flat,
          { ...flat, yaw: 90 },
          ...
          expect(parts.slice(at, at + 3), JSON.stringify({ attitude, alignment })).toEqual([
            'board',
            'board-mark',
            'arrow',
          ])
