...
import { CliTimeoutError, CliUnavailableError, readDiff } from './io'
...
describe('readDiff against the mock FC', () => {
  it('reports the setup of the default mock: telemetry, the receiver and ESC sensor ports, two modes', async () => {
    ...
    expect(report.sections.map((s) => s.title)).toEqual(['feature', 'serial', 'aux'])
    ...
    expect(countDifferences(report)).toBe(6)
  })
